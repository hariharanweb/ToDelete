...
import com.calatrava.shell.NativeJSContainer;
...
public class RhinoService extends Service {
	...
	private NativeJSContainer nativeJSContainer;
	...
	public void initRhino() {
		...
		try {
			...
			Object wrappedRegistry = Context.javaToJS(
					PageRegistry.sharedRegistry(), mScope);
			nativeJSContainer = new NativeJSContainer();

			ScriptableObject.putProperty(mScope, "nativeContainer",
					Context.javaToJS(nativeJSContainer, mScope));
			ScriptableObject.putProperty(mScope, "pageRegistry",
					wrappedRegistry);
			ScriptableObject.putProperty(mScope, "androidRuntime", this);

			Object wrappedAjaxRequestManagerRegistry = Context.javaToJS(
					AjaxRequestManager.sharedManager(), mScope);
			ScriptableObject.putProperty(mScope, "ajaxRequestManagerRegistry",
					wrappedAjaxRequestManagerRegistry);
			...
			try {
				...
				Log.d(TAG,
						"Interrupted Exception when waiting for JSEvalThread");
				e.printStackTrace();
			}
		} finally {
			Context.exit();
	...
	public void invokeFailureCallback(String requestId, int statusCode,
			String responseBody) {
	...
	class JSEvalThread extends Thread {
		...
		public void callJsFunction(String function) {
			String js = "{0}();".replace("{0}", function);
			dispatchJs(js);
		}
		...
		public void callJsFunction(String function, String[] args) {
			...
			String js = "{0}({1});".replace("{0}", function).replace("{1}",
					sb.toString());
		...
		public void triggerEvent(String proxy, String eventId,
				String[] extraArgs) {
			StringBuilder sb = new StringBuilder("");
			...
			String js = "calatrava.inbound.dispatchEvent('{0}', '{1}'{2});"
					.replace("{0}", proxy).replace("{1}", eventId)
		...
		public void ajaxSuccessfulResponse(String requestId, String json) {
			String js = "calatrava.inbound.successfulResponse('{0}', '{1}');"
					.replace("{0}", requestId).replace("{1}", json);
			dispatchJs(js);
		}

		public void ajaxFailureResponse(String requestId, int statusCode,
				String responseBody) {
	...
	public NativeJSContainer getNativeJSController() {
		return nativeJSContainer;
	}
}
